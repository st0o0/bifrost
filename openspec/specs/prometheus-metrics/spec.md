...
All per-peer metrics SHALL use a `peer` label containing the first 8 characters of
the peer's base64-encoded public key. The system SHALL NOT expose endpoint IPs or
hostnames as label values.

#### Scenario: Peer label value
- **WHEN** a peer has public key `aB3dEfGhIjKlMnOpQrStUvWxYz1234567890abcdefg=`
- **THEN** the `peer` label value is `aB3dEfGh`
